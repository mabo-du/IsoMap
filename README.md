...
- **Column Mapping Engine**: Uses a 6-stage fallback pipeline (exact → cache → fuzzy → embeddings → distribution) to accurately map source columns to target repositories.
...
- **Export & Provenance Engine**: Emits standard REST JSON arrays, NeoToma GeoJSONs, Excel files, and outputs fully transparent, reproducible JSON provenance audit logs in the Frictionless Data format.
